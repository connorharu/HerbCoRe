...
- tool.py: non-interactive version of the tool—executes database/speciesLink methods by providing all arguments in a single submission;
- interactive.py: interactive, incremental version of the tool—executes methods related to the database, speciesLink, or Leipzig by providing arguments step-by-step;
- main_f.py: implementation of database and speciesLink methods;
- synonym.py: direct method for checking synonyms of provided scientific names against the [Leipzig catalogue](https://www.nature.com/articles/s41597-020-00702-z);
- deduplicating.py: direct method for ranking the authors of scientific names found in the records.
...
Install dezoomify-rs 2.12.3 [here](https://github.com/lovasoa/dezoomify-rs/releases/tag/v2.12.3) – PLACE IT IN THE ```herbcore_tool``` DIRECTORY, NOT IN ```downloader-specieslink-master```! Do the same for the `pipeline` directory, if used.
...
Each one represents a method of the tool. For a better description of how each method works, read the section on the tool or run ```python tool.py METHOD_NAME -h```. Entering a number other than those listed will return you to the previous menu.
...
```python tool.py -h``` will display the following:
```

tool.py's methods interface:
...
```python synonym.py -h``` will display the following:
```
...
```python deduplicating.py -h``` will display the following:
```
...
For example, ```python tool.py metadata -h``` will display the following:
```
usage: tool.py metadata [-h] --api_key API_KEY [--name NAME] [--id ID]
...
```python
# examples of tool.py:
python tool.py metadata --name "Secretaria Estadual" --id "400"
python tool.py participants
python tool.py instituition --acronym "USP" --id "393" --lang "en"  
python tool.py collection --acronym "ESA" --id "8" --lang "pt-br"
python tool.py dataset --id "8"
python tool.py records --filters family=piperaceae barcode="FURB38192" --table tabela_exemplo
python tool.py export --filters family=piperaceae --table tabela_exemplo --colums "coluna_exemplo" --output_csv_path resultados.csv
python tool.py update --filters stateprovince="São Paulo" --update_values="Santa Catarina" --table tabela_exemplo
...
# examples of synonym.py:
python synonym.py extract --txt teste-08-04.txt --csv aaa.csv
python synonym.py fuzzy --csv aaa.csv --output ccc.txt --max_distance 0.1 
python synonym.py fuzzy --csv aaa.csv --output ccc.txt (max_distance é opcional!)
python synonym.py fuzzy_line --csv teste.csv --tabela registros_biodiversidade --coluna scientificname_NOVO --status status_plantas --max_distance 0.1

# examples of deduplicating.py:
python deduplicating.py --csv nomes2-15-06.csv --ranking 5 --similar 100 --txt nome.txt
```
## About the classifier folder:
...
- svm_interactive.py: used to call the methods from svm2.py interactively, similar to how herbcore_tool is used.
...
Similar to the herbcore_tool menu, you can choose from execution options:
...
The tool prompts you for a set of commands via the terminal to perform the requested operations. If you are unfamiliar with the necessary search parameters, you can use the ```-h``` flag to view the included help comments. It is recommended to read the section on the continuous tool in herbcore_tool to understand how the ```-h``` help guide works.
...
python svm2.py testing_dataset --arq1 imagens-saida/f1.npy --arq2 imagens-saida/f2.npy --arq3 imagens-saida/f3.npy --dir_imagens imagens-saida --kernel linear --c 0.1 --folds 5 --novo_arq1 amostragem/f1.npy --novo_arq2 amostragem/f2.npy --novo_arq3 amostragem/f3.npy --matriz 1

# just training
python svm2.py training --arq1 imagens-saida/f1.npy --arq2 imagens-saida/f2.npy --arq3 imagens-saida/f3.npy --dir_imagens imagens-saida --kernel linear --c 0.1 --folds 5 --matriz 1

# separate images into subdirectories based on a training set
python svm2.py organize_images --arq1 imagens-saida/f1.npy --arq2 imagens-saida/f2.npy --arq3 imagens-saida/f3.npy --dir_origem "teste/" --dir_destino "teste-org/ --kernel linear --c 0.1"

# extracted feature vectors
python svm2.py verify_dimensions --arq1 imagens-saida/f1.npy --arq2 imagens-saida/f2.npy --arq3 imagens-saida/f3.npy
```
